...
def _(mo, np):
    ...
    x_range = mo.ui.range_slider(
        0, 4 * np.pi, value=[0, 4 * np.pi], step=0.1, label="X-axis Range"
    )
    return (
        amplitude,
        frequency,
...
def _(
    ...
    if wave_type.selected_key == "sine":
        ...
        y = amplitude.value * (
            2
            * (
                frequency.value * x / (2 * np.pi)
                + phase.value / (2 * np.pi)
                - np.floor(
                    frequency.value * x / (2 * np.pi) + phase.value / (2 * np.pi) + 0.5
                )
            )
        )
    ...
    wave_chart = (
        alt.Chart(wave_df)
        .mark_line(color="steelblue", size=2)
        .encode(
            x=alt.X("x:Q", title="x"),
            y=alt.Y(
                "y:Q",
                title="y",
                scale=alt.Scale(
                    domain=[wave_df["y"].min() - 0.5, wave_df["y"].max() + 0.5]
                ),
            ),
        )
        .properties(width=600, height=300, title=f"{wave_type.value.title()} Function")
    )

    if show_points.value:
        points = (
            alt.Chart(wave_df.iloc[::10])
            .mark_point(color="red", size=30)
            .encode(x="x:Q", y="y:Q")
        )
        wave_chart = wave_chart + points

    wave_chart = wave_chart.configure_axis(grid=show_grid.value)
...
def _(mo):
    ...
    dist1_param1 = mo.ui.number(
        start=-10, stop=10, value=0, step=0.1, label="Parameter 1 (μ or min)"
    )
    dist1_param2 = mo.ui.number(
        start=0.1, stop=10, value=1, step=0.1, label="Parameter 2 (σ or max)"
    )
    ...
    dist2_param1 = mo.ui.number(
        start=-10, stop=10, value=-2, step=0.1, label="Parameter 1 (μ or min)"
    )
    dist2_param2 = mo.ui.number(
        start=0.1, stop=10, value=2, step=0.1, label="Parameter 2 (σ or max)"
    )
...
def _(
    ...
    data1 = generate_distribution(
        dist1_type.selected_key,
        dist1_param1.value,
        dist1_param2.value,
        sample_size.value,
    )
    data2 = generate_distribution(
        dist2_type.selected_key,
        dist2_param1.value,
        dist2_param2.value,
        sample_size.value,
    )
    ...
    if show_comparison.value:
        ...
        dist_chart = (
            ...
            .properties(width=600, height=300, title="Distribution Comparison")
        )
    else:
        dist_chart = (
            alt.Chart(df1)
            ...
            .properties(
                width=600, height=300, title=f"{dist1_type.value.title()} Distribution"
            )
        )
...
def _(mo):
    ...
    gallery_dropdown = mo.ui.dropdown(
        options=["Option A", "Option B", "Option C"], value="Option A", label="Dropdown"
    )
    gallery_radio = mo.ui.radio(
        options=["Red", "Green", "Blue"], value="Red", label="Radio Buttons"
    )
...
def _(
    ...
    widget_tab = mo.vstack(
        [
            ...
            mo.md(
                "Explore all available Marimo UI widgets. Each widget's current value is displayed below it."
            ),
            mo.hstack(
                [
                    mo.vstack(
